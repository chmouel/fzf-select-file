...
__fzf_select_file() {
    local exa_extras=$@
    local choices=(${(f)"$(${exa_cmd} ${=exa_extras} ${=ZSH_FZF_SELECT_FILE_EXA_ARGS} .|fzf --bind="ctrl-v:change-preview-window(nohidden)" --preview-window=hidden --preview="${ZSH_FZF_SELECT_FILE_FZF_PREVIEW}" ${=ZSH_FZF_SELECT_FILE_FZF_ARGS})"})
    local space=""
    local goend=0
    (( ${#choices} )) || return 1
    if [[ -n ${BUFFER} ]];then
